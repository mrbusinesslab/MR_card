...
SHEET_CONFIG = [
    {"key": "1WIIhNOsLcJhq0wjwrGSmpaVPGOe3NBlHszcfNQOalLc", "label": "BNI 磐石"},
    {"key": "1tsSKZp8dkjz54jMUXuZ4rVvOHLRngQdqjUDqOFi_meU", "label": "BNI 建築組"},
]
...
def _fetch_sheet_rows(sheet_id, sheet_name):
    url = (
...
def _load_people():
    ...
    for cfg in SHEET_CONFIG:
        for sheet_name in ("資料蒐集", "顧問快診", "盲蒐"):
            try:
                rows = _fetch_sheet_rows(cfg["key"], sheet_name)
            except Exception as exc:
                print(f"WARNING: 無法讀取 {cfg['label']}／{sheet_name}: {exc}")
                continue
...
def available_categories(person, has_card=False):
    ...
    if has_value(person, "顧問快診", ["網路看到的你 vs 實際的你", "第一眼印象", "別人會怎麼認識你", "搜尋現況", "目前優勢", "外界可能看不懂的地方", "缺少的關鍵資產", "對生意可能造成的影響", "顧問現場一句話", "建議下一步"]):
        categories.append(("diagnosis", "顧問快診"))
    if has_value(person, "盲蒐", ["姓名搜尋結果", "公司搜尋結果", "自然出現的品牌／名稱", "搜尋到的平台／資產", "外界第一眼會怎麼理解", "本人與公司／品牌關聯是否看得懂", "搜尋干擾／同名問題", "盲搜時沒有自然出現的資訊", "盲搜結論", "品牌落差"]):
...
def category_text(person, category):
    ...
    sections = {
        ...
        "diagnosis": ("顧問快診", diagnosis, ["網路看到的你 vs 實際的你", "第一眼印象", "別人會怎麼認識你", "搜尋現況", "目前優勢", "外界可能看不懂的地方", "缺少的關鍵資產", "對生意可能造成的影響", "顧問現場一句話", "建議下一步"]),
        "blind": ("盲搜／品牌落差", blind, ["姓名搜尋結果", "公司搜尋結果", "自然出現的品牌／名稱", "搜尋到的平台／資產", "外界第一眼會怎麼理解", "本人與公司／品牌關聯是否看得懂", "搜尋干擾／同名問題", "盲搜時沒有自然出現的資訊", "盲搜結論", "後續真實資料比對", "品牌落差", "查核日期", "備註"]),
    }
