const CARD_JSON_URL = "../../ai-MRbot/templates/case1_小如如/card_小如如.json";
const VERSION = { label: "v1.0", date: "2026-09-12" };
...
document.querySelector("#versionLabel").textContent = VERSION.label;
const versionDate = document.querySelector("#versionDate");
versionDate.dateTime = VERSION.date;
versionDate.textContent = VERSION.date.replaceAll("-", "/");
...
async function loadCard() {
  const separator = CARD_JSON_URL.includes("?") ? "&" : "?";
  const response = await fetch(`${CARD_JSON_URL}${separator}v=${encodeURIComponent(VERSION.date)}`, { cache: "no-store" });
